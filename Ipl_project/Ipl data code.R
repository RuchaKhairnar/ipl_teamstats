...
team_name_year <- function(y,start,end=NA){               ##  Creating a function
   ...
   over_dt_year <- data.frame(inning=numeric(),over=numeric() , bowler= character() ,                     ## Creating an empty data frame with following column names
  ...
  for ( a in b){#start:end) {
   ...
   for(x in 1:19){
      ...
      over_dt <- rbind(over_dt ,over_dt_year[ over_dt_year$over==x, ])         ## bind the empty dataset with df and iterate in 'for' loop to take values of over from 1 to 19  
      ...
      mean_total <- over_totalruns %>% 
         group_by(over) %>% summarise(avg_runs=mean(runs),year) %>% unique()         ## Calculating mean of runs scored every over
...
team_name_year(y="Mumbai Indians",start = "2017", end= "2018" )
